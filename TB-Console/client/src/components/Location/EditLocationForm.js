...
class EditLocationForm extends Component {
  ...
  addTags = e => {
    ...
    console.log('tagrequest', tagRequest);
  ...
  handleDelete = data => () => {
    ...
    ).then(data => this.props.getLocations());
  ...
  componentDidMount() {
    ...
    tagManager.getAllTags(this.props.location.accountId).then(data => {
      //console.log('og tags', this.state.tags);
      let tagFilter = this.state.tags.map(tag => {
        return tag.tagId;
      });
      let filteredTags = data.filter(tag => !tagFilter.includes(tag.tagId));
      //console.log('filtered', filteredTags);

      this.setState({
        availableTags: filteredTags
      });
      //console.log('tag state', this.state.availableTags, this.state.tags);
    });
  }
    ...
    return (
      <>
        ...
        <div className='tagManager'>
          ...
          />
          <Button onClick={e => this.addTags()}>add Tags</Button>
        </div>
                  ...
                  <div className='doubleFormRow'>
                    ...
                    >
                      ...
                      <NativeSelect
                        ...
                        onChange={this.handleChange('primaryCategory')}
                        input={
                          <OutlinedInput
